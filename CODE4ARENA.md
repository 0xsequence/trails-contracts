...
- **CallsPayload behavior.** The router composes approvals, swaps, bridges via Sequence V3 CallsPayload format. Stress revert bubbling, partial‑success semantics (when upstream sets `behaviorOnError = IGNORE`), and ensure approvals can’t be stranded in a half‑updated state.
