...
contract TrailsLiFiValidatorTest is Test {
    ...
    function test_IsSwapDataValid_WithCallTo() public view {
        ...
        assertTrue(TrailsLiFiValidator.isSwapDataValid(swapData), "should be valid with callTo");
    }

    function test_IsSwapDataValid_WithApproveTo() public view {
        LibSwap.SwapData memory swapData = baseSwapData;
        swapData.approveTo = address(0x1);
        assertTrue(TrailsLiFiValidator.isSwapDataValid(swapData), "should be valid with approveTo");
    }

    function test_IsSwapDataValid_WithFromAmount() public view {
        LibSwap.SwapData memory swapData = baseSwapData;
        swapData.fromAmount = 1;
        assertTrue(TrailsLiFiValidator.isSwapDataValid(swapData), "should be valid with fromAmount");
    }

    function test_IsSwapDataValid_Invalid() public view {
        LibSwap.SwapData memory swapData = baseSwapData;
        assertFalse(TrailsLiFiValidator.isSwapDataValid(swapData), "should be invalid");
    }
    ...
    function test_IsSwapDataArrayValid_WithOneValidSwap() public view {
        ...
        assertTrue(TrailsLiFiValidator.isSwapDataArrayValid(swapDataArray), "should be valid with one valid swap");
    }
    ...
    function test_IsSwapDataArrayValid_WithMultipleSwaps_OneValid() public view {
        ...
        assertTrue(TrailsLiFiValidator.isSwapDataArrayValid(swapDataArray), "should be valid with one of many valid");
    }

    function test_IsSwapDataArrayValid_EmptyArray() public pure {
        LibSwap.SwapData[] memory swapDataArray = new LibSwap.SwapData[](0);
        assertFalse(TrailsLiFiValidator.isSwapDataArrayValid(swapDataArray), "should be invalid for empty array");
    }
    ...
    function test_IsSwapDataArrayValid_OnlyInvalidSwaps() public view {
        ...
        swapDataArray[1] = baseSwapData;
        assertFalse(
            TrailsLiFiValidator.isSwapDataArrayValid(swapDataArray), "should be invalid with only invalid swaps"
        );
    }
    ...
    function test_IsBridgeDataValid_HappyPath() public view {
        assertTrue(TrailsLiFiValidator.isBridgeDataValid(baseBridgeData), "should be valid bridge data");
    }
    ...
    function test_IsBridgeDataValid_Invalid_ZeroTransactionId() public view {
        ...
        assertFalse(TrailsLiFiValidator.isBridgeDataValid(bridgeData), "should be invalid with zero transactionId");
    }

    function test_IsBridgeDataValid_Invalid_EmptyBridge() public view {
        ILiFi.BridgeData memory bridgeData = baseBridgeData;
        bridgeData.bridge = "";
        assertFalse(TrailsLiFiValidator.isBridgeDataValid(bridgeData), "should be invalid with empty bridge");
    }

    function test_IsBridgeDataValid_Invalid_ZeroReceiver() public view {
        ILiFi.BridgeData memory bridgeData = baseBridgeData;
        bridgeData.receiver = address(0);
        assertFalse(TrailsLiFiValidator.isBridgeDataValid(bridgeData), "should be invalid with zero receiver");
    }

    function test_IsBridgeDataValid_Invalid_ZeroMinAmount() public view {
        ILiFi.BridgeData memory bridgeData = baseBridgeData;
        bridgeData.minAmount = 0;
        assertFalse(TrailsLiFiValidator.isBridgeDataValid(bridgeData), "should be invalid with zero minAmount");
    }

    function test_IsBridgeDataValid_Invalid_ZeroDestinationChainId() public view {
        ILiFi.BridgeData memory bridgeData = baseBridgeData;
        bridgeData.destinationChainId = 0;
        assertFalse(TrailsLiFiValidator.isBridgeDataValid(bridgeData), "should be invalid with zero destinationChainId");
    }
    ...
    function test_IsBridgeAndSwapDataTupleValid_HappyPath_WithSourceSwaps() public view {
        ...
        swapDataArray[0].callTo = address(0x1);
        assertTrue(
            TrailsLiFiValidator.isBridgeAndSwapDataTupleValid(bridgeData, swapDataArray),
