...
contract PausableTest is Test {
  ...
  function test_pause_revertsWhenAlreadyPaused() external {
    ...
    vm.prank(owner_);
    vm.expectRevert(IPause.EnforcedPause.selector);
    pausable.pause();
  }
  ...
  function test_transferOwnership_updatesOwner() external {
    PausableHarness pausable = _newPausable();

    vm.prank(owner_);
    vm.expectEmit(true, true, false, true);
    emit OwnershipTransferred(owner_, newOwner);
    pausable.transferOwnership(newOwner);

    assertEq(pausable.owner(), newOwner);

    vm.prank(owner_);
    vm.expectRevert(abi.encodeWithSelector(Ownable.OwnableUnauthorizedAccount.selector, owner_));
    pausable.setOperator(operator, true);
