...
contract TrailsTokenSweeperTest is Test {
    ...
    function test_validateOpHashAndSweep_native_success() public {
        ...
        bytes memory data = abi.encodeWithSelector(
            TrailsTokenSweeper.validateOpHashAndSweep.selector, address(0), recipient
        );
        ...
        // Verify slot still set
        bytes32 stored = vm.load(holder, slot);
        assertEq(stored, TEST_SUCCESS_VALUE);
    }
    ...
    function test_validateOpHashAndSweep_erc20_success() public {
        ...
        bytes memory data = abi.encodeWithSelector(
            TrailsTokenSweeper.validateOpHashAndSweep.selector, address(erc20), recipient
        );
