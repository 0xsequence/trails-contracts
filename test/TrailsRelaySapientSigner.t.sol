...
import {TrailsRelaySapientSigner} from "@/TrailsRelaySapientSigner.sol";
...
contract TrailsRelaySapientSignerTest is Test {
    using Payload for Payload.Decoded;

    TrailsRelaySapientSigner public signerContract;
    ...
    function setUp() public {
        relaySolverAddress = TrailsRelayConstants.RELAY_SOLVER;
        // The TrailsRelaySapientSigner is configured with the address of the relay solver.
        signerContract = new TrailsRelaySapientSigner();
    ...
    function test_RecoverSingleRelayCall_ERC20_ValidSignature() public {
        ...
        bytes32 expectedExecutionInfoHash =
            TrailsExecutionInfoParams.getTrailsExecutionInfoHash(attestedExecutionInfos, userSignerAddress);
    ...
    function test_RecoverSingleRelayCall_Native_ValidSignature() public {
        ...
        // The TrailsRelayDecoder expects just the requestId for native transfers.
    ...
    function test_Revert_ApproveToInvalidSpender() public {
        ...
        TrailsExecutionInfo[] memory dummyInfos = new TrailsExecutionInfo[](1);
        dummyInfos[0] = TrailsExecutionInfo({
            originToken: address(mockToken),
            amount: approvalAmount,
            originChainId: block.chainid,
            destinationChainId: block.chainid
        ...
        bytes32 digestToSign = TrailsExecutionInfoParams.getTrailsExecutionInfoHash(dummyInfos, userSignerAddress);
        ...
        // 5. Expect revert
        vm.expectRevert(TrailsRelaySapientSigner.InvalidRelayRecipient.selector);
    ...
    function test_RecoverSingleRelayCall_Native_ValidAttestation() public {
        ...
        attestedExecutionInfos[0] =
            TrailsExecutionInfo({originToken: address(0), amount: amount, originChainId: 1, destinationChainId: 1});
    ...
    function test_recoverSapientSignature_failing_from_user() public {
        ...
        // In the real scenario, the TrailsRelaySapientSigner is configured with the true
        // RELAY_SOLVER address, not the intermediate RelayReceiver contract address.
        TrailsRelaySapientSigner signerContractForThisTest = new TrailsRelaySapientSigner();
        ...
        bytes32 digestToSign =
            TrailsExecutionInfoParams.getTrailsExecutionInfoHash(attestedExecutionInfos, expectedSigner);
    ...
    function test_recoverSapientSignature_revert_from_trace() public {
        TrailsRelaySapientSigner signer = new TrailsRelaySapientSigner();
